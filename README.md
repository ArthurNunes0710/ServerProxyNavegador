...
Para iniciar o Proxy é necessário apenas executar o arquivo ```app.py```, e então na url do seu navegador digitar a porta ```http://localhost:8080/``` + o site que se deseja acessar, por exemplo: ```http://localhost:8080/http://guthib.com```e se estiver bloqueado entrará em uma página específica para os sites bloqueados que apenas diz 'Não' no centro da tela. Além disso, se o site conter algumas das palavras proibidas do arquivo words.json seu conteúdo será filtrado.
